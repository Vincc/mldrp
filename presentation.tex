...
\item Natural language processing: large language models (wChatGPT, Claude, DeepSeek, etc.), machine translation, coding, math
...
\begin{frame}
    ...
    \item Instead of one attention function, transformers use h=8 parallel heads:\\
        $\operatorname{MultiHead}(Q, K, V) = \operatorname{MultiHead}(XW_i^Q, XW_i^K, XW_i^V) = \operatorname{Concat}(head_1, \dots, head_k)W^0$
    \item Each head applies attention to linearly projected versions of $Q, K, V$:\\
        $head_i = \operatorname{Attention}(XW_i^Q, XW_i^K, XW_i^V)$, where $W_i^Q, W_i^K, W_i^V$ are learnable projection matrices.
